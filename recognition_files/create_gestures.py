...
def store_images(g_id):
	...
	while True:
		...
		thresh_delta = cv2.threshold(blur,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)[1]
		thresh_delta = cv2.merge((thresh_delta,thresh_delta,thresh_delta))
		thresh_delta = cv2.cvtColor(thresh_delta, cv2.COLOR_BGR2GRAY)
		thresh_delta = thresh_delta[y:y+h, x:x+w]
		...
		if len(contours) > 0:
			...
			if cv2.contourArea(contour) > 10000 and frames > 50:
				...
				pic_no += 1
				save_img = thresh_delta[y1:y1+h1, x1:x1+w1]
				if w1 > h1:
