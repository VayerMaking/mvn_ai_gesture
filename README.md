...
В папка ```/hub``` се намира файл ```controlserverflask.py```, когато той бъде пуснат(препоръчително на Raspberry Pi, за да може да работи 24/7), той стартира flask сървър, към който се подават get заявки и чрез тях се извършват действия, пердварително добавени във файла ```controlserverflask.py```
